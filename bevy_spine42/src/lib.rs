...
/// // bevy system
/// fn on_spine_event(
///     mut spine_events: MsgReader<SpineMsg>,
    ...
    for (handle_index, (handle, handle_config)) in local.handles.iter().enumerate() {
        if let Some(mut image) = images.get_mut(handle) {
            ...
            if handle_config.premultiplied_alpha {
                if let Some(ref mut image_p) = image.data {
                    for i in 0..(image_p.len() / 4) {
                        ...
                        image_p[i * 4] = (rgba.red * 255.) as u8;
                        image_p[i * 4 + 1] = (rgba.green * 255.) as u8;
                        image_p[i * 4 + 2] = (rgba.blue * 255.) as u8;
                        image_p[i * 4 + 3] = (rgba.alpha * 255.) as u8;
                    }
                }
            }
...
pub mod prelude {
    pub use crate::{
        ...
        SpineMsg, SpineLoader, SpineMesh, SpineMeshState, SpinePlugin, SpineReadyMsg, SpineSet,
