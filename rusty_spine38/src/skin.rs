use crate::{
    c::{
        spSkeletonData, spSkin, spSkin_addSkin_38, spSkin_copySkin_38, spSkin_create_38, spSkin_dispose_38,
        spSkin_getAttachments_38,
    },
...
impl Skin {
    ...
    pub fn new(name: &str) -> Skin {
        ...
        Self {
            c_skin: SyncPtr(unsafe { spSkin_create_38(c_name.as_ptr()) }),
            owns_memory: true,
        }
    }
    ...
    pub unsafe fn add_skin(&mut self, other: &Skin) {
        unsafe {
            spSkin_addSkin_38(self.c_ptr_mut(), other.c_ptr());
        }
    }

    pub fn copy_skin(&mut self, other: &Skin) {
        unsafe {
            spSkin_copySkin_38(self.c_ptr_mut(), other.c_ptr());
        }
    }
    ...
    pub fn attachments(&self) -> Vec<AttachmentEntry> {
        ...
        unsafe {
            let mut entry = spSkin_getAttachments_38(self.c_ptr());
            while !entry.is_null() {
...
impl Drop for Skin {
    fn drop(&mut self) {
        if self.owns_memory {
            unsafe {
                spSkin_dispose_38(self.c_skin.0);
            }
        }
    }
}
