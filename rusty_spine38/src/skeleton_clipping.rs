use crate::{
    c::{
        spSkeletonClipping, spSkeletonClipping_clipEnd_38, spSkeletonClipping_clipEnd2_38,
        spSkeletonClipping_clipStart_38, spSkeletonClipping_clipTriangles_38, spSkeletonClipping_create_38,
        spSkeletonClipping_dispose_38, spSkeletonClipping_isClipping_38,
    },
    c_interface::SyncPtr,
...
impl SkeletonClipping {
    ...
    pub fn new() -> Self {
        Self {
            c_skeleton_clipping: unsafe { SyncPtr(spSkeletonClipping_create_38()) },
            owns_memory: true,
        }
    }
    ...
    pub fn clip_start(&mut self, slot: &Slot, clip: &ClippingAttachment) {
        unsafe {
            spSkeletonClipping_clipStart_38(self.c_ptr_mut(), slot.c_ptr(), clip.c_ptr());
        }
    }

    pub fn clip_end(&mut self, slot: &Slot) {
        unsafe {
            spSkeletonClipping_clipEnd_38(self.c_ptr_mut(), slot.c_ptr());
        }
    }

    pub fn clip_end2(&mut self) {
        unsafe {
            spSkeletonClipping_clipEnd2_38(self.c_ptr_mut());
        }
    }

    #[must_use]
    pub fn is_clipping(&self) -> bool {
        unsafe { spSkeletonClipping_isClipping_38(self.c_ptr_mut()) != 0 }
    }
    ...
    pub unsafe fn clip_triangles(
        ...
        spSkeletonClipping_clipTriangles_38(
            self.c_ptr(),
...
impl Drop for SkeletonClipping {
    fn drop(&mut self) {
        if self.owns_memory {
            unsafe {
                spSkeletonClipping_dispose_38(self.c_skeleton_clipping.0);
            }
        }
    }
}
