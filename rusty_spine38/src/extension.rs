...
//!
//! * `void _spAtlasPage_createTexture_38 (spAtlasPage* self, const char* path);`
//! * `void _spAtlasPage_disposeTexture_38 (spAtlasPage* self);`
//! * `char* _spUtil_readFile_38 (const char* path, int* length);`
//!
...
/// Set `_spAtlasPage_createTexture_38`
///
...
/// Set `_spAtlasPage_disposeTexture_38`
///
...
#[no_mangle]
extern "C" fn _spAtlasPage_createTexture_38(c_atlas_page: *mut spAtlasPage, c_path: *const c_char) {
    let singleton = Extension::singleton();
...
#[no_mangle]
extern "C" fn _spAtlasPage_disposeTexture_38(c_atlas_page: *mut spAtlasPage) {
    let singleton = Extension::singleton();
    let extension = singleton.lock().unwrap();
...
#[no_mangle]
extern "C" fn _spUtil_readFile_38(c_path: *const c_char, c_length: *mut c_int) -> *mut c_char {
    let singleton = Extension::singleton();
    let extension = singleton.lock().unwrap();
    extension.read_file_cb.as_ref().map_or_else(
        || {
            ...
            read(str).map_or(std::ptr::null_mut(), |data| {
                let c_data = unsafe { spine_malloc_38(data.len() as size_t) };
                unsafe {
                    spine_memcpy_38(c_data, data.as_ptr().cast::<c_void>(), data.len() as size_t);
        ...
        |cb| {
            cb(unsafe { CStr::from_ptr(c_path).to_str().unwrap() }).map_or(
                ...
                |data| unsafe {
                    *c_length = data.len() as c_int;
                    let c_data = spine_malloc_38(data.len() as size_t);
                    spine_memcpy_38(c_data, data.as_ptr().cast::<c_void>(), data.len() as size_t);
                    c_data.cast::<c_char>()
                },
            )
