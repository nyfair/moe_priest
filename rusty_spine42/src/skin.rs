use crate::{
    c::{
        spSkeletonData, spSkin, spSkin_addSkin_42, spSkin_copySkin_42, spSkin_create_42, spSkin_dispose_42,
        spSkin_getAttachments_42,
    },
...
impl Skin {
    ...
    pub fn new(name: &str) -> Skin {
        ...
        Self {
            c_skin: SyncPtr(unsafe { spSkin_create_42(c_name.as_ptr()) }),
            owns_memory: true,
        }
    }
    ...
    pub unsafe fn add_skin(&mut self, other: &Skin) {
        unsafe {
            spSkin_addSkin_42(self.c_ptr_mut(), other.c_ptr());
        }
    }

    pub fn copy_skin(&mut self, other: &Skin) {
        unsafe {
            spSkin_copySkin_42(self.c_ptr_mut(), other.c_ptr());
        }
    }
    ...
    pub fn attachments(&self) -> Vec<AttachmentEntry> {
        ...
        unsafe {
            let mut entry = spSkin_getAttachments_42(self.c_ptr());
            while !entry.is_null() {
...
impl Drop for Skin {
    fn drop(&mut self) {
        if self.owns_memory {
            unsafe {
                spSkin_dispose_42(self.c_skin.0);
            }
        }
    }
}
