...
use crate::c::{
    spAtlasFilter, spAtlasFormat, spAtlasRegion, spAtlasWrap, spAtlas_createFromFile_42,
    spTextureRegion,
};
use crate::c_interface::{CTmpRef, NewFromPtr, SyncPtr};
use crate::texture_region::TextureRegion;
use crate::{
    c::{c_int, spAtlas, spAtlasPage, spAtlas_create_42, spAtlas_dispose_42},
    error::SpineError,
};
...
impl Atlas {
    ...
    pub fn new_from_file<P: AsRef<Path>>(path: P) -> Result<Atlas, SpineError> {
        ...
        let c_atlas = unsafe { spAtlas_createFromFile_42(c_path.as_ptr(), null_mut()) };
        if !c_atlas.is_null() {
            Ok(Self {
                c_atlas: SyncPtr(c_atlas),
                owns_memory: true,
...
impl Drop for Atlas {
    fn drop(&mut self) {
        if self.owns_memory {
            unsafe {
                spAtlas_dispose_42(self.c_atlas.0);
            }
        }
    }
}
