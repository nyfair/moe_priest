...
use crate::{
    animation::Animation,
    c::{
        c_void, spAnimationStateData, spAnimationStateData_create_42, spAnimationStateData_dispose_42,
        spAnimationStateData_getMix_42, spAnimationStateData_setMix_42,
        spAnimationStateData_setMixByName_42, spSkeletonData,
    },
...
impl AnimationStateData {
    ...
    pub fn new(skeleton_data: Arc<SkeletonData>) -> Self {
        let c_animation_state_data = unsafe { spAnimationStateData_create_42(skeleton_data.c_ptr()) };
        Self {
            c_animation_state_data: SyncPtr(c_animation_state_data),
            owns_memory: true,
    ...
    pub fn set_mix_by_name(&mut self, from_name: &str, to_name: &str, duration: f32) {
        ...
        unsafe {
            spAnimationStateData_setMixByName_42(
                self.c_ptr(),
                c_from_name.as_ptr(),
                c_to_name.as_ptr(),
                duration,
    ...
    pub fn set_mix(&mut self, from: &Animation, to: &Animation, duration: f32) {
        unsafe {
            spAnimationStateData_setMix_42(self.c_ptr(), from.c_ptr(), to.c_ptr(), duration);
        }
    }

    pub fn get_mix(&mut self, from: &Animation, to: &Animation) -> f32 {
        unsafe { spAnimationStateData_getMix_42(self.c_ptr(), from.c_ptr(), to.c_ptr()) }
    }
...
impl Drop for AnimationStateData {
    fn drop(&mut self) {
        if self.owns_memory {
            unsafe {
                spAnimationStateData_dispose_42(self.c_animation_state_data.0);
            }
        }
    }
}
