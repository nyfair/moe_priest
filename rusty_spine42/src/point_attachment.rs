use crate::{
    bone::Bone,
    c::{
        spAttachment, spPointAttachment, spPointAttachment_computeWorldPosition_42,
        spPointAttachment_computeWorldRotation_42,
    },
...
impl PointAttachment {
    ...
    pub fn compute_world_position(&self, bone: &Bone) -> (f32, f32) {
        ...
        let mut y = 0.;
        unsafe {
            spPointAttachment_computeWorldPosition_42(self.c_ptr(), bone.c_ptr(), &mut x, &mut y);
        }
        (x, y)
    }
    ...
    pub fn compute_world_rotation(&self, bone: &Bone) -> f32 {
        unsafe { spPointAttachment_computeWorldRotation_42(self.c_ptr(), bone.c_ptr()) }
    }
