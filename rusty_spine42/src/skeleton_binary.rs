...
use crate::{
    c::{
        c_uchar, spSkeletonBinary, spSkeletonBinary_create_42, spSkeletonBinary_dispose_42,
        spSkeletonBinary_readSkeletonData_42, spSkeletonBinary_readSkeletonDataFile_42,
    },
...
impl SkeletonBinary {
    ...
    pub fn new(atlas: Arc<Atlas>) -> Self {
        let c_skeleton_binary = unsafe { spSkeletonBinary_create_42(atlas.c_ptr()) };
        Self {
    ...
    pub fn read_skeleton_data(&self, data: &[u8]) -> Result<SkeletonData, SpineError> {
        let c_skeleton_data = unsafe {
            spSkeletonBinary_readSkeletonData_42(
    ...
    pub fn read_skeleton_data_file<P: AsRef<Path>>(
        ...
        let c_skeleton_data = unsafe {
            spSkeletonBinary_readSkeletonDataFile_42(self.c_skeleton_binary.0, c_path.as_ptr())
        };
        if !c_skeleton_data.is_null() {
            Ok(SkeletonData::new(c_skeleton_data, self.atlas.clone()))
        } else {
...
impl Drop for SkeletonBinary {
    fn drop(&mut self) {
        if self.owns_memory {
            unsafe {
                spSkeletonBinary_dispose_42(self.c_skeleton_binary.0);
            }
        }
    }
}
