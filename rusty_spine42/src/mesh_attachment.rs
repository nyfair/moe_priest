use crate::{
    c::{
        c_float, c_ushort, spAttachment, spMeshAttachment, spMeshAttachment_newLinkedMesh_42,
        spMeshAttachment_updateRegion_42, spTextureRegion, spVertexAttachment,
    },
...
impl MeshAttachment {
    ...
    pub fn new_linked_mesh(&self) -> Attachment {
        unsafe {
            Attachment::new_from_ptr(
                spMeshAttachment_newLinkedMesh_42(self.c_ptr()).cast::<spAttachment>(),
            )
        }
    }
    ...
    pub fn update_region(&mut self) {
        unsafe {
            spMeshAttachment_updateRegion_42(self.c_ptr());
        }
    }
