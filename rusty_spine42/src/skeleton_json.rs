...
use crate::{
    c::{
        spSkeletonJson, spSkeletonJson_create_42, spSkeletonJson_dispose_42,
        spSkeletonJson_readSkeletonData_42, spSkeletonJson_readSkeletonDataFile_42,
    },
...
impl SkeletonJson {
    ...
    pub fn new(atlas: Arc<Atlas>) -> Self {
        let c_skeleton_json = unsafe { spSkeletonJson_create_42(atlas.c_ptr()) };
        Self {
    ...
    pub fn read_skeleton_data(&self, json: &[u8]) -> Result<SkeletonData, SpineError> {
        ...
        let c_skeleton_data =
            unsafe { spSkeletonJson_readSkeletonData_42(self.c_skeleton_json.0, c_json.as_ptr()) };
    ...
    pub fn read_skeleton_data_file<P: AsRef<Path>>(
        ...
        let c_skeleton_data =
            unsafe { spSkeletonJson_readSkeletonDataFile_42(self.c_skeleton_json.0, c_path.as_ptr()) };
        if !c_skeleton_data.is_null() {
            Ok(SkeletonData::new(c_skeleton_data, self.atlas.clone()))
        } else {
            let c_error = unsafe { CStr::from_ptr((*self.c_skeleton_json.0).error) };
...
impl Drop for SkeletonJson {
    fn drop(&mut self) {
        if self.owns_memory {
            unsafe {
                spSkeletonJson_dispose_42(self.c_skeleton_json.0);
            }
        }
    }
}
